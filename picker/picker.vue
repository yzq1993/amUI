<template>
	<am-modal width='100%' height='280px' position='bottom' bottom v-model=show @input=close :shadeClose=shadeClose>
		<div class="am-picker">
			<div class="am-head">
	  			<input type="button" :value=closetext @click.prevent=hidePicker>
	  			<h2>{{title}}</h2>
	  		...
	  		<div class="am-ul">
	  			<div class="am-item" v-for="(item,m) in option" @touchmove.prevent=touchmove @touchstart.prevent=touchstart @touchend.prevent=touchend :index=m>
					<ul :style="'-webkit-transform: translate3d(0px, '+topStyle[m]+'px, 0px);transform: translate3d(0px, '+topStyle[m]+'px, 0px);'" :index=m :class=endClass[m]>
